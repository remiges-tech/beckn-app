...
// becknAuthHeader builds a Beckn HTTP Signature Authorization header value.
//
...
func becknAuthHeader(body []byte, keyID, privateKeyBase64 string) (string, error) {
	...
	if err != nil {
		...
		if err != nil {
			return "", fmt.Errorf("decode BAP private key: %w", err)
		}
	}
	...
	default:
		return "", fmt.Errorf("BAP private key length %d is invalid (expected 32 or 64 bytes)", len(keyBytes))
	}
