...
// Process stores the updated contract snapshot received in on_update.
func (s *Service) Process(ctx context.Context, req *OnUpdateRequest) {
	start := time.Now()
	...
	if err := q.UpsertContractSnapshot(ctx, dbsqlc.UpsertContractSnapshotParams{
		TransactionID: txID,
		Action:        dbsqlc.BecknActionOnUpdate,
		ContractID:    contractID,
		Contract:      contractJSON,
	}); err != nil {
		s.lh.WithModule("on_update_svc").Warn().Error(err).Log("upsert contract snapshot failed")
	}
	...
	if err := q.InsertMessageLog(ctx, dbsqlc.InsertMessageLogParams{
		...
		Url:                  strPtr("/api/webhook/on_update"),
		...
		s.lh.WithModule("on_update_svc").Warn().Error(err).Log(fmt.Sprintf("message log insert failed: %v", err))
	}

	s.lh.WithModule("on_update_svc").LogActivity(
		fmt.Sprintf("← IN   ON_UPDATE | txn=%.8s… | bpp=%s | dur=%dms", req.Context.TransactionID, req.Context.BppID, time.Since(start).Milliseconds()),
